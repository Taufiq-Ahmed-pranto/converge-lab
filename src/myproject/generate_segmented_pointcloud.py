...
def get_segment_colors(num_segments):
    ...
    colors[: min(len(distinct_colors), num_segments)] = distinct_colors[
        : min(len(distinct_colors), num_segments)
    ]
    return colors
...
def run_da3_inference(model, image_files, process_res_method="upper_bound_resize"):
    ...
    prediction = model.inference(
        image=image_files, infer_gs=True, process_res_method=process_res_method
    )
...
def depth_to_point_cloud_with_segments(
    depth_map,
    rgb_image,
    intrinsics,
    extrinsics,
...
def refine_alignment_icp_with_segments(
    point_clouds, segment_arrays, voxel_size=0.02, max_correspondence_distance=0.05
):
    """
    ...
    for i in range(1, len(point_clouds)):
        ...
        print(
            f"  Frame {i}: fitness={result.fitness:.4f}, RMSE={result.inlier_rmse:.6f}"
        )

    return refined_pcds, refined_segs


def refine_alignment_multiway_with_segments(
    point_clouds, segment_arrays, voxel_size=0.02
):
    """
    ...
    for source_id in range(n_pcds):
        for target_id in range(
            source_id + 1, min(source_id + 3, n_pcds)
...
def filter_by_multiview_consistency_with_segments(
    point_clouds, segment_arrays, distance_threshold=0.03, min_views=2
):
    """
    ...
    print(
        f"  Kept {len(consistent_points)} / {len(all_points)} points ({100*len(consistent_points)/len(all_points):.1f}%)"
    )

    return consistent_points, consistent_colors, consistent_segments


def statistical_outlier_removal_with_segments(
    points, colors, segments, nb_neighbors=20, std_ratio=2.0
):
    ...
    print(
        f"  Points: {len(points)} → {len(clean_points)} (removed {len(points) - len(clean_points)})"
    )
...
def export_point_cloud_with_labels(points, colors, segments, filepath_base):
    ...
    # Segment colored PLY
    export_segmented_point_cloud_ply(
        points, colors, segments, f"{filepath_base}_segmented.ply"
    )

    # Save labels as numpy file
    np.savez(
        f"{filepath_base}_labels.npz",
        points=points,
        colors=colors,
        segments=segments,
...
def visualize_segmented_point_cloud(
    points, segments, window_name="Segmented Point Cloud"
):
...
def process_pipeline_with_segmentation(
    data_folder,
    conf_thresh=0.4,
    visualize=True,
    clean=True,
    ...
    for i, img_path in enumerate(image_files):
        print(
            f"  Segmenting image {i+1}/{len(image_files)}: {os.path.basename(img_path)}"
        )
    ...
    if use_multiview_filter:
        ...
        final_pts, final_cols, final_segs = (
            filter_by_multiview_consistency_with_segments(
                point_clouds,
                segment_arrays,
    ...
    if visualize:
        ...
        visualize_point_cloud_open3d(
            final_pts, final_cols, window_name="RGB Point Cloud"
        )

        print("Visualizing segmented point cloud...")
        visualize_segmented_point_cloud(
            final_pts, final_segs, window_name="Segmented Point Cloud"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_visualize", action="store_true", help="Disable visualization"
    )
    parser.add_argument(
        "--no_clean",
        action="store_true",
    ...
    parser.add_argument(
        "--icp", action="store_true", help="Use ICP for pairwise alignment refinement"
    )
    parser.add_argument(
        "--multiway",
        action="store_true",
